...
function isBusy(button, kind) {
  ...
  return button.getAttribute('aria-busy') === 'true'
    || (kind === 'start' ? /开始中/.test(text) : /退出中/.test(text));
}
...
function installButtonLifecycle(panel, button, kind) {
  ...
  button.addEventListener('click', () => {
    ...
    const timer = window.setTimeout(() => {
      ...
      renderFeedback(panel, getWaitingRequestFeedback({
        kind,
        pending: false,
        timedOut: true,
...
function syncRequestState(panel, button, kind) {
  ...
  const feedback = getWaitingRequestFeedback({
    kind,
    pending: busy && !timedOut,
    timedOut,
    connected: Boolean(window.__henan50kConnected),
  });
