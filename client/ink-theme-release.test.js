...
assert(behavior.includes("INK_THEME_RELEASE = 'ink-06-mobile-r4'"), 'runtime must expose the mobile operability R4 identifier');
assert(behavior.includes("badge.textContent = '新中式墨韵 · 06'"), 'lobby must visibly confirm the selected theme');
assert(behavior.includes('手机操作优化R4版'), 'release accessibility label must identify the current mobile edition');
