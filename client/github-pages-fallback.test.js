...
assert.equal(pagesConfig.base, '/-K/', 'GitHub Pages build must use the repository base path by default');
...
process.env.PAGES_BASE_PATH = '/pan/50k';
const alternatePagesConfig = (await import(`./vite.config.js?alternate-pages=${Date.now()}`)).default;
assert.equal(alternatePagesConfig.base, '/pan/50k/', 'Alternate Pages builds must normalize a custom nested base path');
