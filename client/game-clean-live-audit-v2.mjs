...
async function enterGame(page) {
  await page.goto(`${target}/?clean-game-v2=${Date.now()}-${Math.random().toString(16).slice(2)}`, {
...
async function inspect(page) {
  return page.evaluate(() => {
    ...
    const shell = one('.game-table-shell');
    const board = one('.game-table-trick-board');
    const dock = one('.game-table-hand-dock');
    const cards = [...document.querySelectorAll('[data-card-id]')];
    const buttons = [...document.querySelectorAll('.game-hand-actions button')].map(node => ({
      text: node.textContent?.trim() || '',
      rect: rect(node),
      logical: logical(node),
    }));
    const cells = [...(board?.children?.[1]?.children || [])].map(node => ({
    ...
    return {
      ...
      selectionLogical: logical(one('.game-hand-selection-status')),
      buttons,
      cells,
...
function verifyLayout(data, label) {
  ...
  assert(data.boardLogical?.scrollHeight <= data.boardLogical?.clientHeight + 2, `${label}:中央出牌区纵向被裁切 ${data.boardLogical?.scrollHeight}/${data.boardLogical?.clientHeight}`);
  assert(data.boardLogical?.scrollWidth <= data.boardLogical?.clientWidth + 2, `${label}:中央出牌区横向被裁切 ${data.boardLogical?.scrollWidth}/${data.boardLogical?.clientWidth}`);
  ...
  for (const [index, cell] of data.cells.entries()) {
    ...
    assert(cell.logical.scrollHeight <= cell.logical.clientHeight + 2, `${label}:行动格${index + 1}内容被裁切 ${cell.logical.scrollHeight}/${cell.logical.clientHeight}`);
    assert(cell.logical.scrollWidth <= cell.logical.clientWidth + 2, `${label}:行动格${index + 1}横向被裁切 ${cell.logical.scrollWidth}/${cell.logical.clientWidth}`);
  }
  ...
  for (const button of data.buttons) {
    ...
    assert(button.logical.width >= 60 && button.logical.height >= 38, `${label}:按钮“${button.text}”触控区域不足 ${button.logical.width}x${button.logical.height}`);
  }
...
async function stressLabels(page, label) {
  ...
  const result = await page.evaluate(() => {
    ...
    const action = document.querySelector('.trick-action-card__header > :last-child')
      || firstCell?.firstElementChild?.lastElementChild;
    ...
    if (!board || !meta || !grid || !firstCell || !action || !selectionOuter || !selectionText) {
      return { missing: { board: !!board, meta: !!meta, grid: !!grid, firstCell: !!firstCell, action: !!action, selectionOuter: !!selectionOuter, selectionText: !!selectionText } };
    }
    ...
    const textBox = node => {
      ...
      return box({ getBoundingClientRect: () => r });
    };
    return {
      board: box(board),
      meta: box(meta),
      ...
      selectionOuter: box(selectionOuter),
...
console.log('clean landscape game audit V2 passed: Chromium/WebKit, three real plays, no board clipping, longest labels visible');
