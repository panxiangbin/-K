...
async function enterGame(page) {
  await page.goto(`${target}/?clean-game-v3=${Date.now()}-${Math.random().toString(16).slice(2)}`, { waitUntil: 'domcontentloaded', timeout: 90_000 });
...
async function playLegalTurn(page, label, number) {
  ...
  for (let attempt = 1; attempt <= 8; attempt += 1) {
    ...
    const selected = await page.evaluate(() => /出牌\(\d+\)/.test(document.querySelector('.btn-play')?.textContent || ''));
    if (!selected) {
...
console.log('clean landscape game audit V3 passed: Chromium/WebKit, three real plays, all cards visible, logical overflow checks passed');