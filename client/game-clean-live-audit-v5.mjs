...
async function enterGame(page) {
  await page.goto(`${target}/?clean-game-v5=${Date.now()}-${Math.random().toString(16).slice(2)}`, { waitUntil: 'domcontentloaded', timeout: 90_000 });
...
async function stressLabels(page) {
  ...
  return page.evaluate(() => {
    ...
    const measureText = (node, container = node) => {
      ...
      const wordSpacing = px(style.wordSpacing);
      const characterGaps = Math.max(0, [...text].length - 1);
      const wordGaps = Math.max(0, text.split(/\s+/).length - 1);
      const textWidth = baseWidth + characterGaps * letterSpacing + wordGaps * wordSpacing;
      const availableWidth = container.clientWidth - px(containerStyle.paddingLeft) - px(containerStyle.paddingRight);
      const lineHeight = px(style.lineHeight) || px(style.fontSize) * 1.2;
      const availableHeight = container.clientHeight - px(containerStyle.paddingTop) - px(containerStyle.paddingBottom);
      return {
        text,
        font: style.font,
        textWidth,
    ...
    return {
      ...
      selectionOuter: logical(selectionOuter),
      selectionText: logical(selectionText),
      metrics: {
        meta: measureText(meta),
        action: measureText(action),
        selection: measureText(selectionText, selectionOuter),
      },
...
function verifyLongLabels(result, label) {
  assert(!result.missing, `${label}:最长牌型目标缺失`);
  const fits = (metric, name) => {
    assert(metric.whiteSpace === 'nowrap', `${label}:${name}未锁定单行显示`);
    assert(metric.textWidth <= metric.availableWidth + 2, `${label}:${name}横向文字超出 ${metric.textWidth.toFixed(2)}/${metric.availableWidth.toFixed(2)}`);
    assert(metric.lineHeight <= metric.availableHeight + 2, `${label}:${name}纵向文字超出 ${metric.lineHeight.toFixed(2)}/${metric.availableHeight.toFixed(2)}`);
  };
  fits(result.metrics.meta, '四王炸弹汇总');
  fits(result.metrics.action, '八张同点炸弹行动标签');
  fits(result.metrics.selection, '普通七张选牌提示');
}
  ...
  for (const engine of engines) {
    ...
    try {
      ...
      try {
        ...
        const longLabels = await stressLabels(page);
        ...
        verifyLongLabels(longLabels, engine.name);
...
console.log('clean landscape game audit V5 passed: Chromium/WebKit, three real plays, all cards visible, longest labels verified by logical canvas text metrics');
