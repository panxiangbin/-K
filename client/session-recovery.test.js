...
try {
  ...
  assert.equal(RECOVERY_TIMEOUT_MS, 12000);
  assert.equal(getRecoveryStatusView('pending', 'auto').text, '正在自动恢复上次房间…');
  assert.equal(getRecoveryStatusView('pending', 'manual').text, '正在继续上次房间…');
  assert.match(getRecoveryStatusView('timeout').text, /记录仍然保留/);

  {
    const target = createRecoveryTarget();
    const tracker = createRecoveryRequestTracker({ target });
    const socket = {};
    tracker.start(socket, { source:'manual', roomId:'111111' });
    assert.equal(target.events[0].type, RECOVERY_STATUS_EVENT);
    assert.equal(target.events[0].detail.status, 'pending');
    assert.equal(target.events[0].detail.source, 'manual');
    assert.equal([...target.timers.values()][0].delay, RECOVERY_TIMEOUT_MS);
    assert.deepEqual(tracker.reject(socket, '房间不存在'), {
      matched:true, shouldClear:true, source:'manual', roomId:'111111',
    });
    assert.equal(target.events.at(-1).detail.status, 'invalidated');
...

  {
    ...
    assert.equal(target.events.at(-1).detail.status, 'retained');
    assert.match(target.events.at(-1).detail.text, /房间记录仍然保留/);
  }

  {
    ...
    const tracker = createRecoveryRequestTracker({ target });
    const socket = {};
    tracker.start(socket, { source:'auto', roomId:'333333' });
    tracker.complete(socket);
    assert.equal(target.events.at(-1).detail.status, 'success');
    assert.equal(target.timers.size, 0);
  }

  {
    ...
    const tracker = createRecoveryRequestTracker({ target });
    const socket = {};
    tracker.start(socket, { source:'manual', roomId:'444444' });
    target.runTimers();
    assert.equal(target.events.at(-1).detail.status, 'timeout');
    assert.equal(target.events.at(-1).detail.roomId, '444444');
    assert.deepEqual(tracker.reject(socket, '房间不存在'), { matched:false, shouldClear:false });
  }

  {
    ...
    const tracker = createRecoveryRequestTracker({ target });
    const socket = {};
    tracker.start(socket, { source:'auto', roomId:'555555' });
    tracker.cancel(socket);
    assert.equal(target.timers.size, 0);
...

  {
    ...
    const storage = createStorage({
      'henan50k:lastRoomId':'666666',
      'henan50k:666666:playerId':'p3',
      'henan50k:666666:playerToken':'t3',
      'henan50k:777777:playerId':'keep',
    });
    const target = {
      document: {
        body:{},
    ...
    assert.equal(invalidateSavedSession({ storage, roomId:'666666', target, source:'auto' }), true);
    assert.deepEqual(storage.snapshot(), { 'henan50k:777777:playerId':'keep' });
  ...
  {
    const storage = createStorage({
      'henan50k:lastRoomId':'888888',
      'henan50k:888888:playerId':'p5',
      'henan50k:888888:playerToken':'t5',
    });
    assert.equal(invalidateSavedSession({ storage, roomId:'999999', target:{}, source:'manual' }), false);
    assert.equal(storage.getItem('henan50k:lastRoomId'), '888888');
  }
  ...
  assert.match(wsSource, /recoveryTracker\.current\.cancel/);
  assert.doesNotMatch(wsSource, /onDisposeSocket:[\s\S]{0,120}recoveryTracker\.current\.complete/);
