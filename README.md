...
多人联网卡牌游戏，3~4人实时对战，支持手机浏览器直接玩，也支持单机练习。
...
| 对子 | 两张相同点数 |
| 三张 | 三张相同点数 |
| 普通四张 | 同点数 4 张但红黑混合，只能压更小的普通四张，不能压单张/对子/三张/炸弹 |
| 同花五十K | 同花色 5 + 10 + K，属于炸弹 |
| 同色四炸 | 同点数 4 张纯黑或纯红才算炸弹；黑四炸 > 红四炸 |
| 八张炸弹 | 两副牌同点数 8 张 |
| 四王炸弹 | 两张大王 + 两张小王，最大 |

**得分牌**：5（5分）、10（10分）、K（10分），共200分。

**炸弹大小**：同花五十K < 同色四炸 < 八张炸弹 < 四王炸弹；同色四炸内部黑色 > 红色，同色再比点数。红黑混合普通四张不是炸弹。
...
- 单机练习：一个真人 + 3 个机器人，机器人会自动出牌、过牌。
- 提示：自动找出当前能出的牌。
- 理牌：把普通牌放左边，把 5 / 10 / K 得分牌和炸弹集中到右边，方便看分和保炸弹。
- 断线重连：同一台手机退出、接电话、刷新后，只要房间还在，会自动用原身份回到房间。
...
可以做微信小程序方向，建议优先用“小程序 web-view 嵌入 H5”的方式试水；如果要完全原生小程序/小游戏，则需要重写页面和通信层。
