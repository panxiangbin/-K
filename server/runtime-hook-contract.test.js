...
assert.ok(
  transformed.includes("require('./bot-ai-lead-resilience').chooseBotMove"),
  '电脑回合必须经过两手残局抗压优化层',
);
assert.ok(
  !transformed.includes("require('./bot-ai-lead-strategy').chooseBotMove(player.hand, room.lastPlay, botContext)"),
  '服务器入口不得绕过抗压优化层直接调用先手策略层',
);
assert.ok(
